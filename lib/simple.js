...
export function HelperConnection(conn, options = {}) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        triggerCommand: {
            /**
            * Memicu fitur dengan memasukkan command
            * @param {Object} m
            * @param {String} command
            */
            async value(m, command) {
        ...
        quoted: {
            get() {
                ...
                if (quoted.viewOnceMessageV2) {
                    // MANUAL
                    let qText
                    if (quoted.viewOnceMessageV2.message.imageMessage) qText = quoted.viewOnceMessageV2.message.imageMessage
                    else if (quoted.viewOnceMessageV2.message.videoMessage) qText = quoted.viewOnceMessageV2.message.videoMessage
                    return Object.defineProperties(JSON.parse(JSON.stringify(qText)),
                        {
                            mediaMessage: {
                                get() {
                                    const Message = ((quoted.viewOnceMessageV2.message.url || quoted.viewOnceMessageV2.message.directPath) ? { ...quoted } : extractMessageContent(quoted)) || null
                                    if (!Message) return null
                                    const mtype = Object.keys(Message)[0]
                                    return MediaType.includes(mtype) ? Message : null
                                },
